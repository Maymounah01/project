...
// 2.1 fetch usser from api
    function fetchUsers(){
        //make use of the browser fetch api
        fetch(url)
        .then ((response) => response.json())
        .then ((data) => { 
            // 2.2 Passing the user datat to the renderUsers function
            renderUsers(data);
        });
    }
...
function renderUsers(usersData){
    ...
    // 3.1 Render all li tag for each of the users
    usersData.forEach((user, index) => {
        ...
        // append the current user li tage to ul tag
        ul.appendChild(li);
    });
...
function searchUsersByUsername(){
    ...
    // Loop throug all the users and render the ones thst mstch the search
